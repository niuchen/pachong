				...
				for(Element objspen:spenel){
  				...
  				//	String ahref=anode.attr("href");//得到第1个单元格的点击url地址
		    		...
		    		System.out.println(" 一个单元格url地址:"+ahrefu);
		    		  ...
		    		  CloseableHttpResponse pingdatapageresponse=  HttpClientUtil.httpGetRequest(client,newpagetable);
		    		  if(pingdatapageresponse==null){
		    			  logger.error(ahrefu+"单元格访问失败. 超时次多过多.跳过去下个单元格");
		    			  continue;
		    		  }
